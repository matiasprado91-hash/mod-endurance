...
function OnChangeBtn()
{
	...
	ParamAdd(param, "SeedName", record.LVDataList[SEED_NAME].szData);								// ????????
	ParamAdd(param, "TomorrowVolumeOfSales", record.LVDataList[TOMORROW_VOLUME_OF_SALES].szData);	// ???? ??????
	...
	ParamAdd(param, "MinCropPrice", record.LVDataList[MINIMUM_CROP_PRICE].szData);					// ????????????
	ParamAdd(param, "MaxCropPrice", record.LVDataList[MAXIMUM_CROP_PRICE].szData);					// ????????????
...
function HandleAddItem(string a_Param)
{
	...
	record.LVDataList[TODAY_VOLUME_OF_SALES].szData=string(TodaySeedTotalCnt);			// ???? ??????
	record.LVDataList[TODAY_PRICE].szData=string(TodaySeedPrice);						// ???? ????
	record.LVDataList[TOMORROW_VOLUME_OF_SALES].szData=string(NextSeedTotalCnt);		// ???? ??????
