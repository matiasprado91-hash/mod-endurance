...
import ext.mods.gameserver.EnduranceConfig;
...
public final class ItemInstance extends WorldObject implements Runnable, Comparable<ItemInstance>
{
	...
	{
		return (!isAugmented() || Config.ALLOW_AUGMENTED_DROP) && _item.isDropable();
	}
	...
	public boolean isDepositable(boolean isPrivateWarehouse)
{
    ...
    {
        if ((!isTradable() && !isAugmented()) || isShadowItem())
            return false;
    }
...
/**
 * Returns the current Endurance value of this item.
 * -1 means the item has not been initialized yet.
 * 0 means the item is broken.
 */
...
/**
 * Sets the current Endurance value.
 */
public void setEndurance(int endurance)
{
	final int newEndurance = Math.max(0, Math.min(endurance, EnduranceConfig.MAX_ENDURANCE));
	
	if (_endurance == newEndurance)
		return;
	
	_endurance = newEndurance;
	
	if (EnduranceConfig.ENDURANCE_ENABLED && isEnduranceItem())
		ItemInstanceTaskManager.getInstance().add(this);
}
...
{
	return isEnduranceItem() && _endurance == 0;
}

/**
 * Returns true if this item participates in the Endurance system.
 */
public boolean isEnduranceItem()
{
    ...
    final int bodyPart = _item.getBodyPart();
    
    return bodyPart == Item.SLOT_HEAD
            || bodyPart == Item.SLOT_CHEST
            || bodyPart == Item.SLOT_FULL_ARMOR
            || bodyPart == Item.SLOT_LEGS
            || bodyPart == Item.SLOT_GLOVES
            || bodyPart == Item.SLOT_FEET
            || bodyPart == Item.SLOT_L_HAND;
}

/**
 * Returns the default Endurance value for this item.
 */
private int getDefaultEndurance()
{
	if (!EnduranceConfig.ENDURANCE_ENABLED)
		return -1;
	
	if (!isEnduranceItem())
		return -1;
	
	return EnduranceConfig.MAX_ENDURANCE;
}
